...
2. **시각화 차트**: 투명한 배경과 모던한 그라데이션 컬러(블루/퍼플)를 사용하여 Altair 또는 Plotly 렌더링을 적용합니다.

#### 4단계: 챗 인터페이스 커스텀
* `st.chat_message` 사용 시 아바타 프로필 아이콘은 CSS를 통해 숨깁니다. 또한 챗봇 타이틀 영역에도 별도의 아이콘 이미지를 렌더링하지 않습니다.
