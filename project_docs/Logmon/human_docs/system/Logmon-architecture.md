...
### 3) logmon-ollama
* **역할**: Gemma 2 2B 모델 및 nomic-embed-text 임베딩 모델을 로컬로 로드하여 임베딩 및 LLM 챗 추론 제공.
...
## 🛠️ 6. 원격 배포 및 이미지 캐시 관리

홈서버 환경으로의 무중단 배포를 지원하기 위해 `deploy.sh` 자동화 스크립트를 제공합니다.
* **캐시 기반 빌드 (기본)**: `deploy.sh`는 빠른 배포 속도를 유지하기 위해 도커 빌드 캐시를 활용해 변경 사항 레이어만 재조립하여 반영합니다.
* **빌드 캐시 강제 무력화**: 간혹 Docker 빌드 캐시 문제로 변경된 내용이 컨테이너 이미지에 정상적으로 업데이트되지 않는 경우, `--no-cache` 옵션을 통해 Docker Compose 빌드 캐시를 완전히 배제하고 신규 빌드를 수행하도록 강제할 수 있습니다.
  ```bash
  ./deploy.sh --no-cache
  ```
